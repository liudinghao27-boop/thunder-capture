...
def dashboard_state(
    ...
    try:
        ...
        logging.getLogger("thunder.api.dashboard").warning("Overview stats failed: %s", exc)
    ...
    for d in all_devices:
        ...
        device_matrix.append({
            ...
            "daily_remaining": max(daily_limit - daily_sent, 0) if daily_limit > 0 else 0,
            "hourly_sent": int((state.wave_sent if state else 0) or 0),
            "hourly_window_started_at": str((state.rate_limited_at if state else "") or ""),
...
def execution_monitor(
    ...
    for s in states:
        ...
        last_payload = last_log.payload if last_log and isinstance(last_log.payload, dict) else {}
        health = s.health if isinstance(s.health, dict) else {}
        ui_tree = last_shot.ui_tree if last_shot and isinstance(last_shot.ui_tree, dict) else {}
        agent_decision = (
        ...
        devices.append({
            ...
            "decision_screenshot": decision_screenshot(agent_decision) or (last_shot.image_path if last_shot else ""),
...
def funnel_analytics(
    ...
    try:
        ...
        logging.getLogger("thunder.api.dashboard").warning("Readiness check failed: %s", exc)
    ...
    if industry_slug:
        try:
            ...
            logging.getLogger("thunder.api.dashboard").debug("Failure distribution query failed: %s", exc)
    ...
    try:
        ...
        logging.getLogger("thunder.api.dashboard").debug("Failure distribution aggregation failed: %s", exc)
        failure_dist = []
